...
Protocol (identical to Protocol 01, but with ³²P):
  1. Baseline: 20 steps at rest (-70 mV)
  2. Stimulus: 50 steps depolarization (-10 mV) 
  3. Recovery: 300 steps at rest (-70 mV)
  Total: 370 steps per run
...
PROTOCOL_NAME = "Isotope Substitution (³²P)"
PROTOCOL_NUMBER = "02"

# Protocol timing (in steps, not milliseconds!)
BASELINE_MS = 20
STIMULUS_MS = 50
RECOVERY_MS = 300
...
OUTPUT_DIR = Path(__file__).parent / "experimental_protocols" / f"protocol_02_isotope_P32_{timestamp}"
...
print("="*80)
print(f"Runs: {N_RUNS}")
print(f"Protocol: {BASELINE_MS} steps rest → {STIMULUS_MS} steps stimulus → {RECOVERY_MS} steps recovery")
print(f"Total duration per run: {TOTAL_MS} steps")
print(f"CRITICAL: Using 100% ³²P (vs ³¹P in Protocol 01)")
...
# KEY EXPERIMENTAL MANIPULATION: Use ³²P instead of ³¹P
params.environment.fraction_P31 = 0.0  # No ³¹P
params.environment.fraction_P32 = 1.0  # 100% ³²P
...
print(f"  Isotope: {params.environment.fraction_P32*100:.0f}% ³²P (EXPERIMENTAL)")
print()
...
if stats_summary['T2_dimer_s']['mean'] > 0:
    ...
    print()
    print("KEY PREDICTION TEST:")
    print(f"  T2 with ³²P: {stats_summary['T2_dimer_s']['mean']:.1f} s")
    print(f"  Expected T2 with ³¹P: ~42s (from Protocol 01)")
    print(f"  Expected reduction: ~10x")
    print(f"  If T2 reduced → Quantum mechanism NECESSARY")
print()
...
ax_ts.set_xlabel('Time (μs)')
ax_ts.set_ylabel('Peak Ca²⁺ (μM)')
ax_ts.set_title('Example Time Series - ³²P Isotope')
...
summary_text = f"""
ISOTOPE SUBSTITUTION (³²P) RESULTS
N={N_RUNS} runs

Key Metrics:
  Dimers:      {stats_summary['dimer_accumulation_nM']['mean']:.2f} ± {stats_summary['dimer_accumulation_nM']['std']:.2f} nM
  T2:          {stats_summary['T2_dimer_s']['mean']:.1f} ± {stats_summary['T2_dimer_s']['std']:.1f} s
  J-coupling:  {stats_summary['j_peak_Hz']['mean']:.1f} ± {stats_summary['j_peak_Hz']['std']:.1f} Hz

Comparison to Protocol 01 (³¹P):
  Expected T2(³¹P): ~42 s
  Measured T2(³²P): {stats_summary['T2_dimer_s']['mean']:.1f} s
  
  Ratio: {42.0/stats_summary['T2_dimer_s']['mean']:.1f}x
  
  If ratio ~10x:
    ✓ Quantum mechanism NECESSARY
    
  If ratio ~1x:
    ✗ Quantum mechanism NOT necessary
"""
...
# Save figure
fig_path = OUTPUT_DIR / 'isotope_statistics.png'
...
with open(summary_path, 'w') as f:
    json.dump({
        ...
        'protocol': {
            'baseline_steps': BASELINE_MS,
            'stimulus_steps': STIMULUS_MS,
            'recovery_steps': RECOVERY_MS,
            'total_steps': TOTAL_MS,
        }
    }, f, indent=2)
...
print(f"  - isotope_statistics.png (visualizations)")
print(f"  - statistics.json (summary statistics)")
print(f"  - raw_data.h5 (all N={N_RUNS} runs)")
print()
print("KEY FINDINGS:")
print(f"  • Isotope: 100% ³²P")
print(f"  • T2 coherence time: {stats_summary['T2_dimer_s']['mean']:.1f} ± {stats_summary['T2_dimer_s']['std']:.1f} s")
print(f"  • Dimer formation: {stats_summary['dimer_accumulation_nM']['mean']:.2f} ± {stats_summary['dimer_accumulation_nM']['std']:.2f} nM")
print()
print("NEXT STEP:")
print("  Compare to Protocol 01 to calculate T2 ratio")
print("  Expected ratio: ~10x if quantum mechanism necessary")
print("="*80)