...
|---|------|--------------------|--------|-------|
| COV-1 | 2026-08-08 | **THE COVARIANCE ROUTE LEAKS — the ~0.75 sign-invariant decode is the honest binding-specific ceiling on THIS design; a directional (magnitude-weighted) readout reopens the abundance channel via CO-ACTIVATION-STRUCTURED trial-to-trial covariance that matched MARGINALS do not close.** Tested the advisor's open Q (PO10 Addendum 2): does a covariance-across-trials statistic recover domain co-membership DIRECTIONALLY without the abundance leak? Post-hoc on the ucB keystone data (`sweep/po10_covariance_readout.py`, NO model runs), reproduced the registered baseline first (sign-agreement `full` 0.750 DEC, bindoff/scramble/lamshort chance — matches the scorer). **Covariance readout = per-trial products of z-scored Δw** (z removes per-cluster abundance mean+scale). **Naive z-product LEAKS on bindoff+scramble+lamshort** via a WHOLE-TRIAL common-mode (high-dimer trials → all clusters co-vary). **Common-mode removed** (subtract each trial's cluster-mean): fixes **bindoff 0.938→0.562 chance** and **lamshort chance**, and corr-structure S: bindoff +0.11 p=0.43, lamshort −0.11 p=0.67 — BUT **scramble STILL DECODES 0.958 (S=+1.96, p=0.000)** while `full`=0.938. **Diagnosis (decisive):** sign-agreement gives scramble **0.000** (binding fully destroyed) vs the covariance's **0.958** — the gap is a co-activation abundance covariance: co-DRIVEN clusters have correlated committed-dimer COUNTS trial-to-trial (same co-activation), a magnitude channel that survives membership-scrambling and needs no binding. **CONCLUSION:** on this design, ANY magnitude-weighted/directional readout reintroduces the leak; the sign-INVARIANCE is precisely what makes the 0.75 decode binding-specific, so **~0.75 is the honest ceiling here — supports 'quantum CONSTRAINS, classical COMPUTES'.** The sign carries the binding (collapse coin); the magnitude carries co-activation abundance; they cannot be separated by re-analysis. **CONSTRUCTIVE (not fundamental-forever):** the leak is that the design matches MARGINALS but not the trial-to-trial CO-ACTIVATION COVARIANCE. A NEW design that also matches the co-activation covariance (co-driven and non-co-driven cluster pairs equal in abundance-covariance) WOULD let a directional covariance readout be binding-specific — that is the next experiment, not a re-analysis. Dopamine's sign-resolution job (F3) still needs a leak-free directional channel; this says the current design cannot supply one. | [GROUNDED, measured — post-hoc; NEGATIVE for the covariance route; ~0.75 ceiling stands here] | `sweep/po10_covariance_readout.py` |
| PO10-2 | 2026-07-22 | **THE FIRST WEIGHT-LEVEL RESULT: the input's pairing structure REACHES THE SYNAPTIC WEIGHTS and is recoverable there, while a classical readout of the identical substrate cannot recover it — a MODEST, DETECTED (not robust) keystone with a clean control ladder.** Unit C, the first measurement at the Δw level (all prior work was substrate: components/cross_w/domains/fidelity). **Task (makes binding NECESSARY):** 4 clusters A,B,C,D in ONE compartment, all within λ_F; two pairings — {AB,CD} vs {AC,BD} — with **marginals matched by construction** (each cluster active the same duration/intensity in both; only *which pairs coincide* differs), so any first-order/scalar mechanism is at chance and only the second-order entanglement partition can separate them. **Readout:** dopamine → joint collapse → per-synapse Δw (each domain a shared random ±1); decode the **sign-invariant pairwise agreement** (LOO-CV vs 2000-shuffle null). Four arms: full / bindoff (independent collapse) / scramble (shuffled domain membership) / lamshort (λ_F=5, below Werner). Built entirely as experiment scripts — **ZERO model .py edits**, off-path by construction (signed readout from the tracker's existing `_find_all_clusters()`/`all_dimers`). **PILOTS both passed:** (A) substrate partition tracks the pairing with hard zeros on off-pairs, no branch-global contam at 10 µm; (B) compartment calcium scalar matched between types (1.2%), the only cross-synapse channel is the entanglement partition. **THE ARC (two negatives, recorded not hidden):** batch 1 (forward-order) → `full` decode **0.667 = NULL**, diagnosed as early-pair decay past the ~57 s Werner floor (only the LATE pair reliable); decoder NOT tuned to force a pass. **Amendment 1 = counterbalance the ordering** (each pairing in both orders → each pair late-position in half its trials → fixes decay asymmetry AND positional confound). Result: **`full` DECODES 0.750 (null p95 0.708)**; controls all chance (bindoff 0.458, scramble 0.000, lamshort 0.500). **Strengthening at delay 5 = NULL (dead end):** `full` 0.667, did not sharpen — consistent with the partition needing decay-driven **self-cleaning** (weak cross-pairing bonds die below Werner → the two pairs sharpen; L·PO7-5), n too small to claim a real delay-dependence. **ROBUSTNESS → DOWNGRADED to DETECTED: doubling to n=12/cell HELD the decode (0.767) with a tighter null (p95 0.674), BUT the bootstrap 95% CI on `full` is [0.562, 0.896] — lower bound BELOW the null p95 ⇒ DETECTED, not robust (POST-REVIEW UPDATE, `L·PO10-2` below).** **Honest ceiling:** modest (~0.77 not ceiling); sign is UNRESOLVED so Δw is *informative not yet consistently useful* (a rule needs a direction — the open architectural question behind Unit C); still the (A) common-cause reading, consistent with the deep-research verdict (classical correlation reservoir computes the same class — quantum CONSTRAINS, classical COMPUTES). Harness `sweep/po10_unitC_*.py`; advisor update `PO10_ADVISOR_UPDATE_UNIT_C_2026-07-22.md`; prereg+Amendment 1 result section. | [GROUNDED, measured — modest, DETECTED weight-level keystone (downgraded from robust); clean ladder] | `L·PO10-2` below |
| PO10-1 | 2026-07-20 | **THE CROSS-SYNAPSE MEDIATING MODE IS DELOCALIZED OVER 15 µm — THE `L·PO9-2` CAVEAT IS DISCHARGED; THE KEYSTONE IS UNCONDITIONAL.** The advisor's point-1 localization estimate (a calculation, NOT a sweep) was run: `PO10_LOCALIZATION_ESTIMATE_2026-07-20.md`. **Verdict: delocalized**, on TWO independent grounds. **(a) Disorder** — evaluated at the actual operating frequency f₀=8 MHz (`omega_0`; the mode whose Q=10 gives L_coh=v·τ≈214 µm): λ=v/f₀=134 µm, reduced wavelength 1/k=21.4 µm > 15 µm (Ioffe–Regel floor); with ξ_loc ∝ ω⁻² (Ishii 1973; Monthus–Garel PRB 81:224208) the localization length at 8 MHz is hundreds of m–km for realistic GTP/GDP disorder — to localize at 15 µm needs σ≈175 (impossible). The scoping's "≲2%" was a band-TOP estimate; at our band-BOTTOM mode the margin is 10⁵–10⁷× larger. The optical ~1 µm number does NOT port (high-frequency excitonic mode). **(b) Structural continuity** (advisor's sharper threat) — the mediator is the shared condensate BUS on the backbone *bundle*, not one MT. In a staggered bundle of N=5–15 MTs a single MT terminus removes only ~1/N (10–20%) of local stiffness — weak disorder, not a wall — so it reduces to (a); the only true wall is a *whole-bundle* discontinuity (branch point), and continuous unbranched dendritic segments run tens of µm > 15 µm. **Mode identity CONFIRMED mechanistically (advisor):** Fröhlich condensation funnels quanta to the band-bottom mode by construction, so 8 MHz *is* the mediator; a GHz mediator is structurally impossible and would independently collapse L_coh (n̄ also ~124× higher at 8 MHz vs 1 GHz, 310 K). **Prediction added:** cross-cluster binding requires both clusters on the same unbranched backbone segment — robust to MT turnover, fails across a branch point. Residual open bet = the standing Q≈10 (AMRIS-class), NOT this. Corroborated v=1074 m/s via E≈1.2–1.9 GPa/ρ≈1.4 g/cm³. Packet `PO9_ADVISOR_PACKET_R3_2026-07-20.md`; log pointer at `L·PO9-2 §5`. **NEXT:** Unit C, the first WEIGHT-level keystone (`PREREG_PO10_UNIT_C_WEIGHT_LEVEL_KEYSTONE.md`). | [GROUNDED — analytic estimate + advisor-confirmed; delocalized] | `L·PO9-2 §5` update |
...
### L·PO10-2 — the first WEIGHT-LEVEL result: the pairing reaches Δw and is recoverable there (classical readout cannot) — a modest, DETECTED keystone with a clean ladder (verdict downgraded from 'robust' post-review) · 2026-07-22 `[GROUNDED, measured — modest, DETECTED weight-level keystone]`

The first measurement at the weight level. Everything prior (components, cross_w, domains, fidelity) was
substrate; Unit C asks whether the computation becomes a **usable synaptic weight pattern** — one a
classical readout of the identical substrate cannot produce. Full detail: `PREREG_PO10_UNIT_C_WEIGHT_LEVEL_
KEYSTONE.md` (+ Amendment 1 + Result); advisor writeup `PO10_ADVISOR_UPDATE_UNIT_C_2026-07-22.md`.

#### 1. Design (makes binding necessary, not incidental)
4 clusters A,B,C,D in ONE compartment, all within λ_F. Two pairings (decoder label): {AB},{CD} vs
{AC},{BD}. **Marginals matched by construction** — each cluster active the same duration/intensity in
both; only *which pairs coincide* differs. So a first-order/scalar mechanism (compartment plateau/calcium)
is at chance by construction; the only second-order (pairing) channel is the entanglement partition
(confirmed by code inspection — Pilot B). Readout: dopamine → joint collapse → per-synapse Δw, each domain
a shared random ±1. Decode the **sign-invariant pairwise agreement** (which clusters co-commit with the
same sign), LOO-CV vs 2000-shuffle null. Built entirely in experiment scripts — **NO model .py edits**
(signed readout replicates `perform_quantum_measurement` from the tracker's `_find_all_clusters()`/
`all_dimers`, plus the ±1 direction).

#### 2. Pilots (both passed before scoring)
- **A (geometry):** substrate partition tracks the pairing, hard zeros on off-pairs across every draw; no
  branch-global contamination at 10 µm in one compartment (the L·ETA-4 risk does not bite at this spacing).
- **B (classical blindness):** compartment-aggregate calcium scalar matched between types (integral 1.2%);
  the only cross-synapse channel to the readout is the entanglement partition. So the classical arm is
  blind to pairing by construction — `bindoff` (arm 2) removes exactly that channel.

#### 3. The arc — two negatives, recorded not hidden
- **Batch 1 (forward order, n≈3/cell) → NULL.** `full` = 0.667 = chance. Diagnosed from the per-draw
  agreements (not by tuning): the EARLY pair's cross-bond decayed below the Werner floor by the 20 s
  readout (early pair ~50–65 s old, past the ~57 s crossing), so only the LATE pair carried reliable
  signal — 2 of 6 features. The registered decoder was NOT swapped for a late-pair-only variant (the
  L·PO5-13 goalpost-move).
- **Amendment 1 — counterbalance the ordering.** Each pairing run in both orderings → each pair is
  late-position (reliably co-signed) in half its trials. Fixes the decay asymmetry AND the positional
  confound (A always early / D always late) → the decode must ride the pairing, not absolute timing.
- **Strengthening at delay 5 → NULL (dead end).** Read out sooner to save the early pair: `full` = 0.667,
  did NOT sharpen. Consistent with the partition needing decay-driven **self-cleaning** (weak cross-pairing
  bonds die below Werner, sharpening the two pairs — L·PO7-5), though 0.667 vs 0.750 is within noise so no
  real delay-dependence is claimed. Recorded as a negative.

#### 4. Result (counterbalanced, delay 20)
...
**Only `full` decodes; all three controls at chance.** `bindoff` chance ⇒ it is the quantum joint collapse,
not independent readout. `scramble` chance ⇒ grouping BY COINCIDENCE, not any grouping. `lamshort` chance ⇒
it depends on binding clearing the Werner floor (ties Δw to the physics PO10-1 settled). **Robustness:**
doubling `full` to n=12/cell held the decode (0.767) while the null tightened (p95 0.708→0.674) → clears by
a wider margin; the 0.75 is not a fluke. (`bindoff` extended to n=12 for symmetry; chance at n=6.)

#### 5. Honest ceiling & what's open
Modest (~0.77, not ceiling); does NOT strengthen at shorter delay (§3). **Sign UNRESOLVED** → Δw is
*informative but not yet consistently useful*: an agreement pattern is information, a learning rule needs a
direction, and nothing in the architecture resolves the per-domain sign — the open architectural question
directly behind Unit C. Still the **(A)** common-cause reading (canonical §5.1), consistent with the
deep-research verdict that a classical correlation reservoir computes the same class — **quantum CONSTRAINS,
...
> **UPDATE 2026-07-20 (PO-10) — THIS CAVEAT IS DISCHARGED. The mode is DELOCALIZED over 15 µm; the
> keystone is no longer conditional.** The localization estimate was run (`PO10_LOCALIZATION_ESTIMATE_
> 2026-07-20.md`) and returns **delocalized** on two independent grounds: (a) *disorder* — evaluated at
> the actual operating frequency f₀=8 MHz (λ=134 µm, 1/k=21.4 µm > 15 µm), ξ_loc ∝ ω⁻² makes the mode
> delocalized by ≥5 orders for any attainable mechanical disorder; the "≲2%" figure above was a band-top
> estimate and does NOT bind at our band-bottom mode. (b) *structural continuity* — the mediator is the
> shared condensate BUS on the backbone *bundle*, so a single MT terminus is only ~1/N (N=5–15) local
> stiffness disorder, not a wall; the true limit is inter-branch backbone length (tens of µm) > 15 µm.
> **Mode identity confirmed mechanistically by the advisor** (Fröhlich condensation funnels to the
> band-bottom mode by construction, so 8 MHz *is* the mediator). Residual open bet = the standing Q≈10
> (AMRIS-class), not this. Prediction added: cross-cluster binding needs both clusters on the same
> unbranched backbone segment. Packet `PO9_ADVISOR_PACKET_R3_2026-07-20.md`. *(Recommend a full
> `L·PO10-1` DECISION RECORD entry — offered, not yet added.)*
...
**Inherited from PO-8 (trusted):** the correlated-domain graph dies at ~20–25 s from **population loss, not decoherence** — mean P_S is still 0.63 at 120 s, intra F=1.000 throughout. PO-8 correctly concluded the bond *release rate* is not the lever. The open question PO-8 escalated: is the ~25 s dimer lifetime real, or an artifact?

#### 1. The diagnosis — one term, and it is bulk chemistry applied in a confined niche
Gap dissolution (`run_theta_burst_45s.py:250`, and identically the within-trial path `ca_triphosphate_complex.py:418`):
```
k_diss = K_CLASSICAL(0.005) · (1 − singlet_excess) · template_enhancement(≈33×)
```
The template ×33 (introduced for the gap by `85d8915` / MO ruling 016) is a **bulk detailed-balance catalyst**: a kinetic catalyst (Tao 2010) must speed both directions. But detailed balance needs a live FORWARD reaction, and during a **silent gap formation is gated OFF** (`ca_triphosphate_complex.py:395` — *"Gates FORMATION only (dissolution unchanged)"*; calcium at baseline, S<1). So during silence the template is a **one-sided loss term** — precisely the "no compensating formation" concern PO-4 raised in Q4-10 and the MO resolved on a premise since superseded (PO-7 brief §5.1.2: this is a driven birth-death NESS, *not* detailed balance; `SUBSTRATE_AUDIT_JUL18:178`: the coherence factor already breaks detailed balance in the same term). At the P_S the substrate holds across a gap, the ×33 pulls effective τ from ~200 s to ~12–46 s — **contradicting the model's own grounded Turhan τ=200 s and the confined-niche A3/D8 measurement** (conserved pool, dissolution returns ions locally via signed-net `_po4_consumed`, τ≈200 s recovery, measured with NO template).

**External corroboration (deep-lit, 21 sources, 3-vote adversarial verification):** across four independent matrix systems (phosphorylated osteopontin, fetuin-A, PILP/anionic polymers, collagen) templates **STABILISE the amorphous phase**, never symmetrically catalyse its dissolution; a dose-response asymmetry (less fetuin-A → faster ripening; nucleation rate unchanged) is one a symmetric catalyst cannot produce. Caveat carried honestly: this measures ACP→crystal *transformation*, not dissolution to free ions, and is bulk/colloidal scale — it fixes the **direction** (template protects, not catalyses loss), not a magnitude. Zero literature exists on cluster persistence on cytoskeletal scaffolds.

#### 2. FIX 1 — drop the template from GAP dissolution only
`k_diss = K_CLASSICAL · (1 − singlet_excess)` in the gap (pre-`85d8915` form). Within-trial dissolution KEEPS the template (formation is active there, so the symmetry is real). Not opt-in: this is a default revert of a 2-day-old change on a superseded premise; making the confined chemistry opt-in would enshrine the artifact.

#### 3. Data-level validation (N=12 free draws, `po8_unitB_trace_readout.py`, λ=5 µm, no seed, 12/12 ignited)
Median by delay (before = PO-8 committed `po8_unitB_results.json`, off arm):

| delay | dimers before→now | domain now | n_cross now | crossF now | largest_frac now |
|---|---|---|---|---|---|
| 0 s | 2580 → 2510 | 436 | 1762 | 0.68 | 0.98 |
| 30 s | — → 2448 | 316 | 1674 | 0.68 | 0.97 |
| **40 s** | — → 2414 | **14** | **451** | **0.53** | **0.46** |
| 120 s | 631 → **2028** | 3 | 410 | 0.53 | 0.03 |

**Population blocker RESOLVED:** 19 % of dimers lost by 120 s (was 76 %). The substrate now ages at the confined ~200 s rate. **The real limiter is now visible and clean:** the multi-synapse partition collapses at a sharp ~40 s cliff driven by **cross-bond coherence** — F=P_S²·W crossing the Werner floor culls a whole cohort of cross-bridges at the typical spacing (crossF 0.68→0.53, largest_frac 0.97→0.46), NOT population loss. Per-synapse cores (intra F=1.000, population intact) persist to 120 s+. So the eligibility trace is **two-timescale**: per-synapse structure long, cross-synapse binding short (~40 s at λ=5 µm).

#### 4. FIX 2 — λ decoupling (the rate-vs-fidelity category error, one more instance)
`self.coupling_weights = exp(−distances/coupling_length_um)` was ONE matrix feeding BOTH `p_met_agg` (metabolic aggregation, `:1507`) AND `w_spatial` in the Werner fidelity (`:477/:485`). These are different physical lengths: metabolic aggregation gates WHICH synapses co-ignite (diffusive, ~5 µm); the condensate coherence length sets how far an ignited bond stays correlated. Split: `fidelity_length_um` builds a separate `self.fidelity_weights`, fed to the entanglement tracker; `p_met_agg` keeps `coupling_weights`. `_update_entanglement` is byte-unchanged (offpath digest `515772101786800` reproduces); default λ_F==λ_met is bit-identical; λ_F=214 gives W=0.995 at 1 µm (cross≈intra, the advisor's regime) as a knob.

**The λ call (made, not deferred to the advisor):** do NOT assert λ_F=214. That is the *ballistic* ceiling (vτ in a clean medium); physiological disorder localises the mode far shorter (the optical-superradiance ~1 µm floor). The measured 0.815 fidelity ceiling is **circular** (= exp(−1/5)), so there is no empirical pin either way, and picking the value that makes the ceiling vanish is the named "makes it behave better" failure. λ_F is bounded ~1 µm (localised) … ~214 µm (ballistic); the pin is the contested MT-Q / AMRIS coherence bet. So λ_F is **the independent variable of Unit B** — sweeping it maps whether the ~40 s cross-synapse cliff slides toward the 74–107 s window, turning a hidden assumption into the experiment's axis.

#### 5. What this establishes, and what is next
- The readout-time keystone is now RUNNABLE: the substrate survives to a realistic delay, which it did not before (PO-8's object was erased by the artifact, not answered).
- The computational object is a **two-timescale trace**: per-synapse cores (long) vs cross-synapse binding (~40 s at λ=5). The λ_F sweep decides whether cross-synapse credit assignment reaches a behavioural delay.
- **Blocker for the scored keystone:** per-synapse stimuli on `net.step` (Q2) — needed for the synchronous-vs-staggered density-matched contrast — is still unbuilt. That is the next build, then Unit B (λ_F × delay × input-condition, synapse-level Q_act with the decomposition-null guard, verdict demonstrated failing first).
- Commits: `b30c59c` (both fixes), `1db91f8` (N=12 traces). Seeding remains dormant/regression-only.

### L·PO8-1 — ⚠ **WITHDRAWN BY ITS AUTHOR, 2026-07-20, hours after posting. DO NOT CITE.** The measurement is an artifact of a protocol I invented; the conclusion is not supported.

> **WHY IT IS WITHDRAWN — four failures, in the order they happened.**
>
> 1. **I never read `docs/PO7_TECHNICAL_BRIEF_2026-07-20.md`**, which the PO-8 kickoff names
>    explicitly as required reading ("the physics in equations"). I read the advisor *review* and
>    skipped the brief. Everything below follows from that.
> 2. **The brief fixes `τ_dimer = 200 s`** (§5.1, in the `k_release = 1/T₂ + 1/τ_dimer` derivation)
>    and states that at the corrected release rate slots recycle **~0.7× within the 100–200 s
>    coherence window**. The physics therefore *expects* the graph to persist across that window.
>    My entry claimed a **~7 s** trace. That contradiction alone should have stopped me.
> 3. **I invented a drive-then-quiet protocol no one in this programme uses**, slamming glutamate to
>    0 and voltage to −70 mV. Measured consequence, from my own recorded data:
>    `peak_conc` decays with **τ ≈ 42 s** — ~5× faster than the brief's `τ_dimer = 200 s`. And
>    `step_population` sets `target_count` from **`np.max(dimer_concentration)` — a PEAK, not an
>    integral**, a quirk already flagged in `L·PO5-11` (`:174`). Collapsing the calcium nanodomain
>    collapses the peak, which culls the population far faster than the intrinsic dimer lifetime.
>    **The "fast chemistry" I reported is my protocol collapsing a peak, not the model's chemistry.**
> 4. **It contradicts validated model behaviour** — the commitment gate reproduces commit-rate decay
>    100% → ~55% **over 0–120 s** (`quantum-biology-primer`). A model whose graph is gone by 30 s
>    cannot produce that. I did not check my result against known behaviour before writing it up.
>
> **What survives:** nothing of the conclusion. The *mechanism* observation is still true and
> narrow — population is slaved to `peak_conc` and culled dimers take their bonds with them
> (`_remove_all_bonds_for_dimer`), so "write-once" does not protect a bond whose dimer is culled.
> That is worth knowing when designing a readout protocol. It is **not** evidence about the
> eligibility-trace lifetime.
>
> **System integrity checked and intact:** network-path regression digest reproduces exactly
> (`515772101786800`, `PO7_U11_MODE=offpath`), and **zero system `.py` files were modified** — every
> Python file this PO added is a new `sweep/po8_*.py` probe. The damage was to this log, not the code.
>
> Kept in place per the append-only rule. The original text follows, unedited.

### L·PO8-1 (WITHDRAWN — see box above) — the eligibility trace has a ~7 s half-life, set by FAST CHEMISTRY, not by the 216 s coherence: at a realistic dopamine delay there is nothing left to read · 2026-07-20 `[WITHDRAWN — protocol artifact]`
...
statement is that brief; this entry is the provenance record for how it was reached.

#### 1. Unit 14 — the clique + EM pathway manufactured 97% of the graph
`provenance_bonding` already replaces the 100 ms clique and skips the phenomenological EM pathway
(both established unphysical: LOCC-non-entangling; no microscopic Hamiltonian). 2×2 over provenance
× spin:

| config | E | mean deg | free nuclei | components | largest_frac |
|---|---|---|---|---|---|
| clique+EM, spin OFF | 369,740 | 715.16 | 0.00 | 1 | 1.000 |
| clique+EM, spin ON | 2,031 | 3.93 | 0.07 | 184 | 0.112 |
| provenance, spin OFF | 66 | 0.13 | 3.87 | 23 | 0.017 |
| provenance, spin ON | 66 | 0.13 | 3.87 | 23 | 0.017 |

Clique+EM account for **3.80 of 3.93 bonds/dimer**. With them gone ~3.87 of 4 nuclei are free, and
**spin-resolution becomes a no-op** (identical on/off) — the monogamy violation was entirely their
artefact. (Provenance's own K=2 yields only 66 bonds among 1022 dimers: a dimer claims ≤2 events
but a bond needs two dimers claiming the SAME event, and most events are claimed once and expire.)

#### 2. The release rate — a NESS, and 96× too slow
Entanglement generation is not microscopically reversible, so this is a driven birth–death NESS,
not detailed balance. The decay rate is therefore not free:
`k_release = 1/T₂ + 1/τ_dimer = 1/216 + 1/200 = 9.63e-3/s`, τ_release = 103.8 s. The model runs
~1e-4/s (τ = 10,000 s) — **96× too slow**, which is what makes the graph write-once. Internal
check: `model6_parameters.py:427` puts P_S crossing the Werner floor at 107.0 s for T_singlet=216 s;
the independently derived τ_release is 103.8 s. (Correction recorded: I first used
`get_effective_coherence_time('dimer')`=25 s, an orphaned code path; 216 s is the value the code
runs, `dimer_particles.py:391`.)

#### 3. Unit 15 + the threshold — monogamy cannot prevent percolation
Freeing the budget (provenance, spin) vs the clique+EM+intra-spin run: the **fraction** of
post-ignition samples showing cross-structure is **identical at 19.9%**; only severity improves
(4.6× fewer cross-bonds, percolates half as often, no permanent latch). So the spin budget never
determined whether structure exists. It collapses at **mean cross-degree ≈ 1** (structured window
~0.15, percolated end ~1.4) — the Erdős–Rényi giant-component threshold — while the monogamy bound
is 4. Cross-bonds are **bridges**; bridges percolate far below the density at which the spatially
clustered intra bonds do (which is why Unit 9's single-synapse graph stayed fragmented at mean
degree 3.93).

#### 4. Unit 13 — slot order is provably irrelevant; pair order is not yet isolated
Permuting which of the 4 slots a bond claims → **bit-identical** partition (E, components,
largest_frac, frustration all unchanged). Bonds/step is bursty (mean 10.2, p90 1, max 840; 90% of
bonds form in the 10% of steps that burst), so pair-presentation order *could* matter and is the one
remaining order question.

#### 5. Three framing corrections (all from the reviewer, recorded not patched)
- **Frustration is a MATCHING DEFICIENCY, not H¹.** Hall's condition; under CKW, LP-infeasibility
  over a capacity polytope. Neither is cohomology. **`L·PO7-3`'s "H¹ obstruction" is RETRACTED.**
  The ℂ¹⁶ partial-trace sheaf is genuine but answers spin-state consistency — a different question.
- **Binary occupancy is an UPPER bound, not lower.** CKW: cost = τ = (2F-1)². F=0.55 ⇒ τ=0.01
  (~100 bonds/nucleus); weak bonds are nearly free. So the 491,566 refusals OVERCOUNT and the binary
  rule may OVER-fragment. Sustaining even the degree-715 graph under CKW needs F≲0.537 throughout.
- **The run-to-run variation is the PHYSICS.** The system is stochastic by construction; its output
  is a distribution. Whether the condensate ignites at all varies across free draws. Seeding it to
  get a clean/igniting run selects the outcome — not done. P(ignition) is a first-class finding. The
  seeding capability (`57ccd75`) is dormant, software-regression only.

#### 6. The ensemble (Unit 17, N=16, ZERO seeding) — the decisive measurement
Free-running, `net.step` drive, 20 s/draw. **The bridges are ABOVE the Werner floor, so the giant
component is REAL, not an artefact of unweighted connectivity.**

- **P(ignition) = 16/16 = 1.000.** peak η distribution: min 0.334, median 0.439, max 0.494.
- **largest_frac: min 0.936, median 0.968, max 0.992** — a giant component dominates every draw.
  n_multi (clusters spanning ≥2 synapses) 1–7 per run.
- **Bridge fidelity** (1,557 bridges, cross edges whose removal raises the component count):
  **median F ≈ 0.672**, p10 0.552, max 0.815; τ median 0.119; only **9.5% below F=0.55.** Pooled over
  all 28,673 cross edges: median F 0.679, ceiling 0.815, 8.5% near the floor.
- **VERDICT — INTERMEDIATE.** The bridges are clearly off the Werner floor (only ~9.5% near it), so
  π₀ is NOT counting negligible-entanglement edges as structure — the keystone is genuine. But they
  are NOT the strong regime either: no edge across ~29k exceeds **F = 0.815**, so the correlation is
  modest (τ ≈ 0.12, ~12% of maximal), not maximal.

#### 7. ⚠ CORRECTION — "P(ignition) is a coin-flip" was WRONG, a drive-path artefact
An earlier reading (Units 16, and the framing note in §5 above as first written) claimed ignition
is marginal/seed-dependent. **That was two errors compounded:** (a) Unit 16 drove synapses via
per-synapse `s.step()` + manual `tr.step()`, but **`_update_backbone_field()` — the ONLY setter of
`_backbone_eta` — is called exclusively inside `net.step()` (`multi_synapse_network.py:1286`)**, so
that path leaves η pinned at 0 by construction and "did not ignite" was a bug, not physics; (b)
Unit 12 on the correct `net.step` path showed seed-dependence only because it measured at t=10 s,
the ignition ONSET, where η is just crossing threshold. On the correct path at 20 s, ignition is
**reliable (16/16)**. The stochasticity that IS real lives in the DEGREE and TIMING of ignition and
in largest_frac / n_multi across free draws — not in whether ignition occurs. The
stochasticity-is-the-physics principle stands; the specific "rare event" claim does not. Probes
driving via per-synapse `s.step()` must call `net.step()` (or invoke `_update_backbone_field`) or
the condensate is dead — a reusable gotcha for the next worker.
